  ...
  cart.forEach((items) => {
    ...
    <div class="product-info js-product-info-${items.id}">
      ...
      <div>
        ...
        <p>Quantity:${items.quantity} <span class="update" data-pid=${
      items.id
    }>Update</span><input class="quantity-input js-quantity-input-${
      items.id
    }"><span class="save-quantity-link" data-pid=${
      items.id
    }>Save</span><span class="delete" data-pid=${items.id}>Delete</span></p>
      </div>
      <div class="delivery-options">
        <div class="choose-option">Choose a delivery option:</div>
        <div class="option1">
          <input type="radio" class="radio1 js-radio" data-pid=${
            items.id
          } data-sp=0 name="${items.id}">
          <div>
        ...
        <div class="option2">
          <input type="radio" class="radio2 js-radio" data-pid=${
            items.id
          } data-sp=499 name="${items.id}">
          <div>
        ...
        <div class="option3">
          <input type="radio" class="radio3 js-radio" data-pid=${
            items.id
          } data-sp=999
          name="${items.id}">
          <div>
...
let saved = () => {
  document.querySelectorAll(".save-quantity-link").forEach((save) => {
    save.addEventListener("click", () => {
      ...
      const value = Number(
        document.querySelector(`.js-quantity-input-${pid}`).value
      );
      let item;
      cart.forEach((items) => {
        if (pid === items.id) {
